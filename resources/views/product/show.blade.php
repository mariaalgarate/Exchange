...
@section('content')
    <div class="container py-5">
        <div class="row">
            <div class="col-md-6">
                <img src="{{ asset('storage/' . $producto->imagen) }}" alt="{{ auth()->user()->nombre }}" class="img-fluid mb-3">
            </div>
            <div class="col-md-6">
                ...
                <p>Stock: <span style="color: var(--bs-primary);">{{ $producto->stock }}</span></p>
                 ...
                 <p>Categorías:</p>
                     @foreach($producto->categorias as $categoria) <!-- Iterar sobre las categorías -->
                         <li class="badge bg-info mb-5">{{ $categoria->nombre }}</li> <!-- Mostrar el nombre de cada categoría -->
                     @endforeach
                <br>
                @switch($producto->transaccion)
                @case('Intercambio')
                    <!-- Si el producto es para intercambiar -->
                    <button onclick="confirmExchange({{ $producto->id }}, {{ $producto->precio_unitario }})" class="btn btn-primary">Intercambiar</button>                    @break
                @case('Venta')
                ...
                @case('Ambas')
                    ...
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#imageUploaderModal">Intercambiar</button>
                    <a href="{{ route('purchase', ['id' => $producto->id]) }}" class="btn btn-primary">Comprar</a>
                    @break
            @endswitch
                ...
                <div class="row mb-5">
                    <div class="col-12 py-3">
                        <form action="{{ route('addToCart') }}" method="post">
                            ...
                            <input type="number" name="cantidad_producto" min="1" value="{{ $producto->cantidad }}">
        ...
        <div class="card">
            <div class="card-body">
                @forelse ($producto->resenas as $resena)
                    <!-- Mostrar las reseñas -->
                    <div class="mb-3">
                        <p class="text-muted">{{ $resena->created_at }} por {{ $resena->usuario ? $resena->usuario->nombre : 'Usuario eliminado' }}</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const carritoIcono = document.querySelector('.fas.fa-shopping-cart'); // Icono del carrito en la barra de navegación
        ...
        botonAnadirAlCarrito.addEventListener('click', function() {
            ...
            localStorage.setItem('carrito_producto', JSON.stringify({ nombre: nombreProducto, precio: precioProducto }));
        });
        ...
        carritoIcono.addEventListener('mouseover', function() {
            ...
            if (carritoProducto) {
                ...
                carritoIcono.title = `Producto: ${carritoProducto.nombre}\nPrecio: ${carritoProducto.precio}`;
            }
        });
